...
## 4. Pod
Pod 是 k8s 的最小调度单位，Pod 包含一个或多个 Container(容器) ，Pod 中运行的容器是共享网络的，使用的网络模式是 Docker 的 Container 模式，K8s 创建 Pod 时候会先创建一个空的基础容器，然后在使用 Container 模式创建 Pod 中定义的容器，让它们共享基础容器的网络命名空间，所以容器在 Pod 中是共享网络的，可以使用 localhost 就相互访问。
...
```
### 4.2 操作 Pod 的常用命令
1. kubectl apply 命令，根据指定的资源文件更新 Pod，不存在则创建新的 Pod ，常用的可选参数
- -f 指定本地资源文件或者文件流
- -k 指定资源文件的目录
```bash
# 根据指定的资源文件更新或者创建 Pod
kubectl apply -f  resource-file.yml | resouce-file.json | https://xxx/xxx/resource-file.yml
# 根据标准输入创建或者更新资源实例
cat resource-file.yml |  kubectl apply -f - 
