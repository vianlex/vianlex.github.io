...
- -type type： 按文件类型查找，linux 下的文件类型有：
  ...
  - l -符号链接文件
  - s -socket 文件
- -size [+-]size[unit]： 按文件大小查找，如果加[+-]符号表示查找大于小于指定 size 的文件，不加表示查找等于 size 大小的文件，单位支持(c：字节,w：2 字节，b：512 字节，k：1024 字节，M：1024k，G：1024M)，默认单位是 b
...
1. 参数 -t vfstype 指定要挂载的文件类型，可选择值 iso9960(光盘或者光盘镜像)、msdos(DOS 文件系统)、vfat(fat32 文件系统)、ntfs(windows nt ntfs 文件系统)、smbfs(windows 文件网络共享)、nfs(Linux 文件网络共享)。
