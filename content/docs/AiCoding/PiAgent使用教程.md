...
除了 Steering / Follow-up 两种发送方式，输入内容还有这些前缀：
...
### Steering 与 Follow-up：运行中追加指令

Pi 运行时不必干等——agent 干活的过程中你随时可以插话。两种追加方式，对应「纠偏」和「排队」：

**Steering（引导）**：默认按 `Enter` 发送。发现方向偏了立即纠正，消息会在当前工具调用结束后、下一次模型调用前生效，并**打断**本轮剩余的工具调用。
...
**Follow-up（排队）**：默认按 `Alt+Enter`（macOS `Option+Enter`）发送。不打断当前工作，等 agent 完成本轮全部工作后才处理。
