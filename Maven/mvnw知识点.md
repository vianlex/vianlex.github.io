...
mvnw 是 Maven Wrapper 的缩写。当我们项目想使用指定的 Maven 版本，而不是全局安装的 Maven 版本时，我们通过 Maven Wrapper 定制当前项目需要使用的 Maven 版本。
...
# 不指定 maven 版本时，表示项目使用最新的版本的 maven 
mvn wrapper:wrapper
...
注意运行 `mvn wrapper:wrapper` 命令后，并未开始下载指定的 Maven 版本，只有我们在项目目录中运行 `./mvnw` 命令后，才会开始下载项目指定使用的 Maven 版本。我们安装 mvn wrapper 时只是在项目中生成了 `mvnw` 命令和该命令的配置 maven-wrapper.properties 文件，等相关文件，如下。
...
mvnw 命令跟普通 mvn 命令用法以及参数都是相同，我们跟使用 mvn 命令一眼使用 mvnw 命令来构建项目即可。
